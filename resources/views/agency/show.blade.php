            ...
            <div class="pro-agency-hero-card row align-items-center g-0">
                ...
                <div class="col-md-4 d-flex flex-column align-items-center align-items-md-start justify-content-center p-3 p-md-4">
                    <div class="pro-agency-logo-card mb-3 mb-md-4">
                        <img class="pro-agency-logo shadow-sm"
                             src="@agencyLogo($agency->agency_logo)"
        ...
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-lg-10">
                    ...
                    <div class="about-content text-center opacity-90">
                        {{ $agency->agency_description ?? 'No description available for this agency.' }}
                    </div>
            </div>
        </div>
            </div>
                ...
                <div class="col-lg-12">
                    ...
                    <div class="row">
                        <div class="col-lg-8">
                                    ...
                                    <div class="schedule-card-body">
                                        ...
                                        <div class="schedule-actions">
                                            ...
                                            <div class="action-buttons">
                                                <form action="{{ route('search.results') }}" method="POST" style="display:inline;">
                                                    @csrf
                                                    <input type="hidden" name="origin" value="{{ $fare->route->origin }}">
                                                    <input type="hidden" name="destination" value="{{ $fare->route->destination }}">
                                                    <input type="hidden" name="travel_date" value="{{ date('Y-m-d', strtotime('+1 day')) }}">
                                                    <input type="hidden" name="travel_type" value="oneway">
                            ...
                            @foreach($fares as $fare)
                            @php
                                $selCur = session('currency')['code'] ?? $fare->currency;
                                $exRates = session('currency')['rates'] ?? [];
                                $origCur = $fare->currency;
                                $convRate = $exRates[$origCur] ?? 1;
                                $toRate = $exRates[$selCur] ?? 1;
                                $convFare = ($fare->amount / $convRate) * $toRate;
                                $dep = \Carbon\Carbon::parse($fare->departure_time);
                                $arr = \Carbon\Carbon::parse($fare->arrival_time);
                                $dur = $dep->diff($arr);
                            @endphp
                            <div class="modal fade" id="routeModal{{ $fare->id }}" tabindex="-1" aria-labelledby="routeModalLabel{{ $fare->id }}" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered modal-lg">
                                    <div class="modal-content" style="border-radius:1rem;overflow:hidden;">
                                        <div class="modal-header" style="background:linear-gradient(135deg,#1f75d8,#667eea);border:none;padding:1.5rem 2rem;">
                                            <div>
                                                <h5 class="modal-title text-white fw-bold mb-1" id="routeModalLabel{{ $fare->id }}">
                                                    {{ strtok($fare->route->origin ?? '', ',') }} → {{ strtok($fare->route->destination ?? '', ',') }}
                                                </h5>
                                                <small class="text-white opacity-75">Route Details</small>
                                            </div>
                                            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body p-4">
                                            {{-- Journey Timeline --}}
                                            <div class="d-flex align-items-center justify-content-between mb-4 p-3" style="background:#f8faff;border-radius:.75rem;">
                                                <div class="text-center">
                                                    <div class="fw-bold fs-5" style="color:#1f75d8;">{{ $fare->departure_time }}</div>
                                                    <div class="text-muted small">{{ strtok($fare->route->origin ?? 'Origin', ',') }}</div>
                                                </div>
                                                <div class="text-center flex-grow-1 px-3">
                                                    <div class="text-muted small mb-1">{{ $dur->format('%Hh %Im') }}</div>
                                                    <div style="height:2px;background:linear-gradient(90deg,#1f75d8,#667eea);border-radius:2px;position:relative;">
                                                        <i class="bi bi-bus-front" style="position:absolute;top:-10px;left:50%;transform:translateX(-50%);color:#1f75d8;font-size:1.1rem;"></i>
                                                    </div>
                                                    <div class="text-muted small mt-1">Direct Route</div>
                                                </div>
                                                <div class="text-center">
                                                    <div class="fw-bold fs-5" style="color:#667eea;">{{ $fare->arrival_time }}</div>
                                                    <div class="text-muted small">{{ strtok($fare->route->destination ?? 'Destination', ',') }}</div>
                                                </div>
                                            </div>

                                            <div class="row g-3 mb-4">
                                                <div class="col-6 col-md-3">
                                                    <div class="text-center p-3" style="background:#f8faff;border-radius:.75rem;">
                                                        <i class="bi bi-currency-dollar text-primary fs-4 mb-1 d-block"></i>
                                                        <div class="fw-bold">{{ $selCur }} {{ number_format($convFare, 2) }}</div>
                                                        <small class="text-muted">Per Person</small>
                                                    </div>
                                                </div>
                                                <div class="col-6 col-md-3">
                                                    <div class="text-center p-3" style="background:#f8faff;border-radius:.75rem;">
                                                        <i class="bi bi-clock text-primary fs-4 mb-1 d-block"></i>
                                                        <div class="fw-bold">{{ $dur->format('%Hh %Im') }}</div>
                                                        <small class="text-muted">Duration</small>
                                                    </div>
                                                </div>
                                                <div class="col-6 col-md-3">
                                                    <div class="text-center p-3" style="background:#f8faff;border-radius:.75rem;">
                                                        <i class="bi bi-bus-front text-primary fs-4 mb-1 d-block"></i>
                                                        <div class="fw-bold">{{ $agency->buses->count() }}</div>
                                                        <small class="text-muted">Fleet Size</small>
                                                    </div>
                                                </div>
                                                <div class="col-6 col-md-3">
                                                    <div class="text-center p-3" style="background:#f8faff;border-radius:.75rem;">
                                                        <i class="bi bi-star-fill text-warning fs-4 mb-1 d-block"></i>
                                                        <div class="fw-bold">{{ number_format($roundedRating, 1) }}/5</div>
                                            ...
                                            {{-- Amenities --}}
                                            <div class="mb-4">
                                                <h6 class="fw-bold mb-3">Amenities Included</h6>
                                                <div class="d-flex flex-wrap gap-2">
                                                    @foreach([['bi-wifi','Free WiFi'],['bi-snow','AC'],['bi-plug','Power Outlets'],['bi-cup-hot','Refreshments']] as $amenity)
                                                    <span class="badge px-3 py-2" style="background:#eaf2ff;color:#1f75d8;border-radius:2rem;font-size:.85rem;">
                                                        <i class="bi {{ $amenity[0] }} me-1"></i>{{ $amenity[1] }}
                                                    </span>
                                                    @endforeach
                                                </div>
                                            </div>
                                            ...
                                            {{-- Agency Info --}}
                                            <div class="d-flex align-items-center gap-3 p-3" style="background:#f8faff;border-radius:.75rem;">
                                                <img src="@agencyLogo($agency->agency_logo)"
                                                     alt="{{ $agency->agency_name }}"
                                                     style="width:48px;height:48px;border-radius:50%;object-fit:cover;"
                                                     onerror="this.src='https://ui-avatars.com/api/?name={{ urlencode($agency->agency_name) }}&size=48&background=eaf2ff&color=1f75d8'">
                                                <div>
                                                    <div class="fw-bold">{{ $agency->agency_name }}</div>
                                                    <small class="text-muted">{{ $agency->agency_type ?? 'Bus Operator' }}</small>
                                                </div>
                                            </div>
                                        </div>
