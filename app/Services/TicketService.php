...
class TicketService
{
    ...
    public function generateTicketPdf(BusBooking $booking)
    {
        ...
        $agencyLogoPath = null;
        $agencyLogo = $booking->schedule->bus->agency->agency_logo ?? null;
        if ($agencyLogo) {
            if (str_starts_with($agencyLogo, 'http')) {
                // Full URL from Node.js backend — use directly (isRemoteEnabled handles it)
                $agencyLogoPath = $agencyLogo;
            } else {
                $agencyLogoPath = public_path('assets/images/agency/logo/' . $agencyLogo);
            }
        }
