...
class BusBookingController extends Controller
{
    ...
    public function seatSelection($pickup, $dropoff, $scheduleId, $returnScheduleId = null)
    {
        ...
        if ($schedule->bus->layout_id) {
            $seatLayout = BusSeatLayout::where('id', $schedule->bus->layout_id)
                ->value('layout_json');
            $seatLayout = json_decode($seatLayout, true);
        }
        ...
        if ($returnScheduleId) {
            ...
            if ($returnSchedule->bus->layout_id) {
                $returnSeatLayout = BusSeatLayout::where('id', $returnSchedule->bus->layout_id)
                    ->value('layout_json');
                $returnSeatLayout = json_decode($returnSeatLayout, true);
            }
    ...
    public function applyCoupon(Request $request)
    {
        ...
        try {
            ...
            if ($discount->agency_id && $discount->agency_id != $agencyId) {
                ...
                $couponAgency = BusAgencies::find($discount->agency_id);
                $bookingAgency = BusAgencies::find($agencyId);
        ...
        if ($status === 'successful') {
            try {
                ...
                $booking = BusBooking::create([
                    ...
                    'status' => 'confirmed',
                    // 'baggage_fee' => $data['baggage_fee'] ?? 0,
                    // 'extra_bags_fee' => $data['extra_bags_fee'] ?? 0,
                    // 'overweight_fee' => $data['overweight_fee'] ?? 0,
                    // 'bags_per_passenger' => $data['bags_per_passenger'] ?? 0,
                ...
                Notification::create([
                    ...
                    'message' => "A booking was made for schedule ID: {$scheduleId}.",
                ]);
                ...
                $qrPng = base64_encode($writer->writeString($booking->bookingreference));
                ...
                // Fetch passenger details for the email
    ...
    public function dashboard()
    {
        ...
        $dashboardData = Cache::remember('user_dashboard_' . $userEmail, 300, function () use ($userEmail) {
            ...
            ->select('id', 'bookingreference', 'contact_email', 'contact_phone', 
                    'total_amount', 'qr_code', 'status', 'pickup', 'dropoff', 'bus_schedule_id')
