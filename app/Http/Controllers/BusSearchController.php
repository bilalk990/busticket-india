...
class BusSearchController extends Controller
{
    public function searchBus()
    {
        ...
        $agencies = \App\Models\BusAgencies::withCount('routes')->get();

        $busFares = \App\Models\BusFare::with('agency')
        ...
        $routesName = $busFares;
        $busRoutes = $busFares;
        $busRoutesTimes = $busFares;
        $groupedRoutes = $busFares->groupBy('pickup_name');

        return view('bus.search.bus', compact(
            'title',
            'agencies',
            'routesName',
            'busRoutes',
    ...
    public function results(Request $request)
    {
        ...
        $cacheKey = "search_{$originPoint->id}_{$destinationPoint->id}_{$request->travel_date}_{$travelType}";
        $searchResults = Cache::remember($cacheKey, 600, function () use ($originPoint, $destinationPoint, $request, $selectedCurrency, $exchangeRates, $travelType) {
